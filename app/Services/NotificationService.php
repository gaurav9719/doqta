    ...
    public function sendNotificationNew($sender, $receiver, $type, $data)
    {
        ...
        try {
            ...
            if ($notification->save()) {
                ...
                if ($receiver['is_muted'] != 1) {

                    $communityNotifications = [9, 10, 11, 12, 13, 14, 15];

                    if (in_array($type, $communityNotifications)) {

                        $member = GroupMember::where('user_id', $receiver['id'])->where('group_id', $data['community_id'])->first();
