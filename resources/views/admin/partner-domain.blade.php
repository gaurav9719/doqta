...
@section('content')
    ...
    <main>
        ...
        <div class="modal fade" id="confirm_status_modal_domain" tabindex="-1" aria-labelledby="exampleModalLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header justify-content-center">
                        <h6 class="mb-0" style="text-align: center;width: 100%;" id="message_status_domain">Are you sure
                            to
                            change
                            the status of
                            this domain?</h6>
    ...
    <script type="text/javascript">
        ...
        function changeStatusDomain(domain_ids, is_status) {
            ...
            if (is_status == 1) {
                $('#message_status_domain').html('Do you really want to deactivate this domain?')
            } else {
                $('#message_status_domain').html('Do you really want to activate this domain?')
            }
        ...
        function confirm_status_domain() {
            ...
            $.ajax({
                url: `{{ url('admin/partners-domain/${domain_id}') }}`,
                method: 'PUT',
                data: {
                    "_token": `{{ csrf_token() }}`
                },
                success: function(response) {   
                    ...
                    if (response.is_active == 1) {
                        $('.status_domain' + response.id).html(
                            '<span class="badge badge-success text-bg-success " >Whitelisted</span>');
                        $('.status_btn_domain' + response.id).html(
                            '<button class="btn btn-lg btn-icon" onclick="changeStatusDomain(' + response
                            .id +
                            ',' + response.is_active +
                            ')" title="Make Inactive"><i class="material-icons">block</i></button></div>');
                        ...
                        $('.status_domain' + response.id).html(
                            '<span class="badge badge-danger text-bg-danger" >Blacklisted</span>');
                        $('.status_btn_domain' + response.id).html(
                            '<button class="btn btn-lg btn-icon" onclick="changeStatusDomain(' + response
                            .id +
                            ',' + response.is_active +
